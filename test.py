...
model.fit(trainX, trainY, epochs=1, batch_size=2, verbose=2)
# Estimate model performance
trainScore = model.evaluate(trainX, trainY, verbose=0)
print(
    "Train Score: MSE: {0:0.3f} RMSE: ({1:0.3f})".format(
        trainScore, math.sqrt(trainScore)
    )
)
testScore = model.evaluate(testX, testY, verbose=0)
print(
    "Test Score: MSE: {0:0.3f} RMSE: ({1:0.3f})".format(testScore, math.sqrt(testScore))
)
# generate predictions for training and forecast for plotting
trainPredict = model.predict(trainX)
testForecast = model.predict(testX)
f = np.concatenate((np.full(look_back - 1, np.nan), trainPredict[:, 0]))
plt.plot(dataset, label = "Dataset")
plt.plot(np.concatenate((np.full(look_back - 1, np.nan), trainPredict[:, 0])), label = "train" )
plt.plot(np.concatenate((np.full(len(train) - 1, np.nan), testForecast[:, 0])), label = "test")
plt.legend()
plt.show()